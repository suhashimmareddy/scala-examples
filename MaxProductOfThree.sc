...
def solutin(A:Array[Int]):Int = {
  var d = 0
  var l = List[Int]()
  if(A.length<=2) return -1
  else 
  {
    for(p<-0 to A.length-3)
    {
      for(q<- p+1 to A.length-2)
      {
        for(r<- q+1 to A.length-1) 
        {
          d = p*q*r
          l = l:+d
        }
...
solutin(Array(-3,1,2,-1,5,6)) 60
solutin(Array(0,0,0)) 0
solutin(Array(3,5,7,9,10,-12,-25)) 120
solutin(Array(-3,-20,-1,-6,-12,-12)) 60
